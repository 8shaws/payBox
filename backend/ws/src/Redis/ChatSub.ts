...
export class ChatSub {
  ...
  private reverseSubscriptions: Map<
    string,
    { [clientId: string]: { clientId: string; ws: WebSocket } }
  >;
  ...
  constructor() {
    ...
    this.reverseSubscriptions = new Map<
      string,
      { [clientId: string]: { clientId: string; ws: any } }
    >();
  ...
  subscribe(friendshipId: string, clientId: string, ws: any) {
    ...
    if (
      Object.keys(this.reverseSubscriptions.get(friendshipId) || {}).length ===
      1
    ) {
  ...
  unsubscribe(clientId: string, channelId: string) {
    this.subscriptions.set(clientId, [
      ...(this.subscriptions.get(clientId) || []).filter(
        (id) => id !== channelId,
      ),
    ]);
