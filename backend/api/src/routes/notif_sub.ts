...
notifSubRouter.post("/subscribe", async (req, res) => {
  try {
    ...
    if (id) {
      const { auth, endpoint, expirationTime, p256dh } = SubscibeValid.parse(
        req.body,
      );
      const { status, id: subId } = await insertSub(
        auth,
        endpoint,
        expirationTime,
        p256dh,
      ...
      await Redis.getRedisInst().notif.cacheNotifSub<NotifSubType>(
        `notif:${id}`,
        {
          auth,
          endpoint,
