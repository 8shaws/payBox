...
settingsRouter.post("/nets", async (req, res) => {
  try {
    ...
    if (id) {
      ...
      const { testMode, btcNet, ethNet, solNet } = TestModeSetSchema.parse(
        req.body,
      );

      const { status } = await updateTestmode(
        id,
        testMode,
        solNet,
        ethNet,
...
settingsRouter.get("/", async (req, res) => {
  try {
    ...
    if (id) {
      const isCached =
        await Redis.getRedisInst().settings.getSettingsCache<Settings>(
          `${id}:settings`,
        );
      ...
      await Redis.getRedisInst().settings.cacheSettings(
        `${id}:settings`,
        settings,
        60 * 60 * 24,
      );
...
settingsRouter.get("/exp_pref", async (req, res) => {
  try {
    ...
    if (id) {
      const cache = await Redis.getRedisInst().settings.getClientPref(
        `${id}:pref`,
      );
      if (cache) {
        return res.status(200).json({
