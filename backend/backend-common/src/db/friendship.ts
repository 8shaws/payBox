...
export const requestFriendship = async (
  ...
  if (checkFriendshipRes.friendship.length > 0) {
    return {
      ...
      friendshipStatus: checkFriendshipRes.friendship[0]
  ...
  if (response.insert_friendship_one?.id) {
    return {
      ...
      status: dbResStatus.Ok,
      friendshipStatus: response.insert_friendship_one
        .status as FriendshipStatus,
    };
  }
  return {
  ...
  if (response.update_friendship?.returning[0].id) {
    return {
      ...
      friendship: {
        ...
        status: response.update_friendship?.returning[0]
          .status as FriendshipStatus,
        ...
        friend:
          response.update_friendship?.returning[0].client1.id === clientId
            ? (response.update_friendship?.returning[0].client2 as Friend)
            : (response.update_friendship?.returning[0].client1 as Friend),
      },
      //@ts-ignore
      to:
        response.update_friendship?.returning[0].client1.id === clientId
          ? (response.update_friendship?.returning[0].client2
              .username as string)
          : (response.update_friendship?.returning[0].client1
              .username as string),
    };
  }
  return {
...
export const putFriendshipStatus = async (
  ...
  if (response.update_friendship?.returning[0].id) {
    return {
      status: dbResStatus.Ok,
      friendshipStatus: response.update_friendship?.returning[0]
        .status as FriendshipStatus,
      //@ts-ignore
      to:
        response.update_friendship?.returning[0].client1.id === clientId
