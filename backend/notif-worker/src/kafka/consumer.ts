      ...
      eachMessage: async ({ topic, partition, message, heartbeat, pause }) => {
        try {
            ...
            case TopicTypes.Notif:
              switch (payload.type) {
                ...
                case NotifTopics.FriendRequestRejected:
                  await notifyFriendRequestRejected(
                    payload.to,
                    payload.from,
                    payload.friendshipId,
                  );
                ...
                default:
                  console.log(
                    `No handler in topic: ${topic} for type: ${payload.type}`,
                  );
                  break;
              }
              break;
          ...
          this.consumer.commitOffsets([
            { topic, partition, offset: message.offset as string },
          ]);
          await heartbeat();
        } catch (error) {
          if (error) {
