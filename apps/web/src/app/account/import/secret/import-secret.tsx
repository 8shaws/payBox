...
export const ImportSecret = ({ jwt }: { jwt: string }) => {
  ...
  const [error, setError] = useState<{ msg: string; index: number } | null>(
    null,
  );
  ...
  function onSubmit(data: z.infer<typeof GetAccount>) {
    const call = async () => {
      ...
      const {
        ...
        accounts,
        msg,
      }: { status: responseStatus; accounts: ChainAccount[]; msg: string } =
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="">
            ...
            <CardFooter className="flex flex-col gap-y-2">
              ...
              <Button
                type="submit"
                onClick={() =>
                  form.setValue("secretPhrase", secretPhrase.join(" "))
                }
              >
                Import Account
              </Button>
