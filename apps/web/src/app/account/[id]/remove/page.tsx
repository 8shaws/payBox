...
const deleteAccount = async (jwt: string, accountId: string) => {
  try {
    const response = await fetch(
      `${BACKEND_URL}/account?accountId=${accountId}`,
      {
        method: "delete",
        headers: {
    ...
    <div className="flex justify-center items-center">
      <Card className="w-[400px]">
        <CardHeader>
          ...
          <CardDescription>
            We recommend to save the memonic for this account before deleting...
          </CardDescription>
        </CardHeader>
        <CardContent>
          <CardDescription>
            <ScrollArea className="h-[100px] w-[350px] rounded-md border p-4 text-accent-foreground">
              Even though you are deleting this account, you can always recover
              it using the memonic in paybox or any other wallet.
            </ScrollArea>
          </CardDescription>
        </CardContent>
        <CardFooter className="flex justify-between">
          <Link href={`/account/${params.id}/secretPhrase`}>
