...
export function AccountDeleteButton({ jwt }: { jwt: string }) {
  ...
  const onDialogAction = async () => {
    const { status, msg }: { status: responseStatus; msg: string } =
      await fetch(
        `${BACKEND_URL}/accounts?accountId=${pathname.split("/")[3]}`,
        {
          method: "DELETE",
          headers: {
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            account though you can import the account again in any web3 wallet
            using its memonic.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
