...
export const PrivateKeyFrom = ({
  ...
  function onSubmit(data: z.infer<typeof AccountGetPrivateKey>) {
    const call = async () => {
      const {
        ...
        sol: { privateKey: string };
        eth: { privateKey: string };
        hashPassword: string;
        msg: string;
    ...
    toast.promise(call(), {
      ...
      success: ({ status, sol, eth, hashPassword }) => {
        setPrivateKey([
          {
            network: "Solana",
            privateKey: decryptWithPassword(sol.privateKey, hashPassword),
          },
          {
            network: "Ethereum",
            privateKey: decryptWithPassword(eth.privateKey, hashPassword),
          },
        ]);
        setOpen(true);
  ...
  return (
    <>
      ...
      {open && <PrivateKeyDialogBox open={open} setOpen={setOpen} />}
    </>
  );
};
