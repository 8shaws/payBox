"use client";
import { CommandMenu } from "@/src/components/combo-box";
import { Button } from "@/src/components/ui/button";
import { langs } from "@/src/lib/contants";
import { updateLocale } from "@/src/lib/helper";
import { getLocaleName } from "@/src/lib/utils";
...
function LocaleButton({ locale }: { locale: Locales }) {
    ...
    <>
      ...
      >
        <Button variant={"secondary"} className="w-full justify-between">
          ...
          <span className="text-muted-foreground flex gap-x-1 items-center">
            {getLocaleName(locale)} <ChevronRight className="w-4 h-4" />
          </span>
        </Button>
      </div>
      <CommandMenu
