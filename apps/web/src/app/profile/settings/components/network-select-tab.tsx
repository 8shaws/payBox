...
import { BitcoinCluster, BtcNets, EthCluster, EthNets, SolCluster, SolNets } from '@paybox/common'
...
function NetworkTab({
    ...
    const [selectObject, setSelectObject] = React.useState<{ value: SolCluster | EthCluster | BitcoinCluster, label: string }[]>()
    useEffect(() => {
        switch (chain) {
            case "Solana":
        ...
        <>
            ...
            >
                <div className="w-10 flex items-center justify-center">
                    {chain === "Solana" ? <SolanaIcon className="w-6" /> : chain == "Bitcoin" ? <BitcoinIcon className='w-8 h-8' /> : <EthIcon className="w-4" />}
                </div>
                ...
                <div className="copy flex flex-row gap-x-2 items-center">
                    <Select open={open} onOpenChange={setOpen} onValueChange={setNet}>
                        <SelectTrigger className="border-none focus:ring-0 outline-none hover:border-none focus:outline-none">
                            <SelectValue placeholder={selectObject?.find(({ value, label }) => value == net)?.label} />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectGroup>
                                <SelectLabel>{chain} Networks</SelectLabel>
                                {selectObject && selectObject.map(({ label, value }) => {
                                    return (
                                        <SelectItem
                                            value={value}
                                            onClick={() => {
